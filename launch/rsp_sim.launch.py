...
def generate_launch_description():
    ...
    xacro_file = os.path.join(pkg_path,'description','robot_sim.urdf.xacro') # main xacro
    # robot_description_config = xacro.process_file(xacro_file).toxml()
    robot_description_config = Command(['xacro ', xacro_file, ' use_ros2_control:=', use_ros2_control, ' sim_mode:=', use_sim_time])
    
    # robot_state_publisher node on topic "robot_description"
    params_robot_state_publisher = {'robot_description': robot_description_config, 'use_sim_time': use_sim_time}
